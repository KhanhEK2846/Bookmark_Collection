# Assembly

---

## Code Converter

- [Compiler Explorer](https://godbolt.org/)

## Documments

- [Assembler language reference](https://www.ibm.com/docs/en/aix/7.2?topic=aix-assembler-language-reference)
- [Coding for A.R.C.](https://albert.ias.edu/server/api/core/bitstreams/d47626a1-c739-4445-b0d7-cc3ef692d381/content)
- [High Level Assembler - Opcodes overview](https://bixoft.nl/english/opcd00.htm)
- [HLASM](https://www.ibm.com/docs/en/zos/2.1.0?topic=hlasm)
- [no starch press](https://nostarch.com/search/Assembly)
- [RJH coding](http://www.rjhcoding.com/avr-asm-tutorials.php)
- [x86 Assembly Language Reference Manual](https://docs.oracle.com/cd/E19641-01/802-1948/802-1948.pdf)

---

[<kbd> Back </kbd>](./readme.md)
